...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       String s1="4D442E2E02B70006152330010003";
        String[] strings = s1.split("(?<=\\G.{4})");
        Log.i(TAG,"s1-------------->:"+Arrays.toString(strings));
        ...
        byte[]byArrays=AdanGPS.hexStringToByteArray(hex2);  //b[]=[77, 68, 46, 46, 2,
        Log.i(TAG,"b[]="+Arrays.toString(byArrays));
//        16进制byte数组转换成10进制
        AdanGPS.dHex(TAG,byArrays,byArrays.length); // 4D 44 2E 2E 02 B7
//        判断是否以4D442E2E开头
        if(byArrays[0]==0x4D&&
            byArrays[1]==0x44&&
            byArrays[2]==0x2E&&
            byArrays[3]==0x2E){
//            Log.i(TAG,"right data..");
//            获取消息长度
            int msgSize=((byArrays[4]&0xff)<<8)|(byArrays[5]&0xff);
            Log.i(TAG,"msgSize:"+msgSize);
            if(msgSize==byArrays.length-9){
                Log.i(TAG,"get application");
//                获取application msg
                int applicationId=((byArrays[10]&0xff)<<8)|(byArrays[11]&0xff);
                Log.i(TAG,"applicationId:"+applicationId);
                switch (applicationId){
                    ...
                    case 0x3003:
                        Log.i(TAG,"网络信息查询");
                        break;
                    default:Log.i(TAG,"applicationId---------->"+applicationId);
                }
            }
            else
                Log.i(TAG,"get application error,byArrays.length-9="+(byArrays.length-9));
        }else
            Log.i(TAG,"error data..");
    }//end onCreate
    private void decodeGpsMsg(String msg) {
//        msg = StringUtil.stringToHexString(msg);
        String[] strings = msg.split("(?<=\\G.{28})");
        Log.i(TAG,"分割后"+ Arrays.toString(strings));
        String header = strings[0];
        String data = strings[1];
        byte[] headerBytes = AdanGPS.hexStringToByteArray(header);
        byte[] dataBytes = AdanGPS.hexStringToByteArray(AdanGPS.removeCheckSum(data));
        AdanGPS.dHex("decodeMsg", headerBytes, headerBytes.length);
        AdanGPS.dHex("decodeMsg", dataBytes, dataBytes.length);
        int messageId = ((headerBytes[12] & 0xff) << 8) | (headerBytes[13] & 0xff);
        Log.i("decodeMsg", "messageId : " + messageId);
        switch (messageId) {
            case 0x0001://TBOX_GNSSSetReq
                //用于请求，不用于接收
                break;
            case 0x0002://TBOX_GNSSSSetAck
                break;
            case 0x0003://TBOX_GNSSReport
//                decodeTBOX_GNSSReport(dataBytes);
                Log.i(TAG,"解码gnssreport");
                break;
        }
    }
}